...
router.route("/fichas")
    ...
    .post((req, res) => {
        ...
        const que_drogra_alimento = req.body.que_drogra_alimento

        db.getConnection((err, connection) =>{
            if(err) throw err
            console.log(`conected as id ${connection.threadId}`)
    
            connection.query('INSERT INTO ficha(rut,fecha_nacimiento,fecha_ficha,padre_con_vida,enfermedad_padre,madre_con_vida,enfermedad_madre,paciente_enfermo,tratamiento_paciente,medicamentos,medicamentos_cinco_años,alergico_droga_alimento,que_drogra_alimento) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)', [rut,fecha_nacimiento,fecha_ficha,padre_con_vida,enfermedad_padre,madre_con_vida,enfermedad_madre,paciente_enfermo,tratamiento_paciente,medicamentos,medicamentos_cinco_años,alergico_droga_alimento,que_drogra_alimento],
            (err, rows) =>{
                connection.release() // return the connection to pool
